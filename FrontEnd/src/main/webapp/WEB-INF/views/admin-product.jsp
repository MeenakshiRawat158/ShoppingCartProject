			...
			<div class="panel-body">
				<c:choose>
					<c:when test="${empty product.id }">
						<c:url var="addAction"
							value="manage_product_create"></c:url>

					</c:when>
					<c:otherwise>
						<c:url var="addAction"
							value="manage_product_update"></c:url>
					</c:otherwise>
				</c:choose>
				<form method="POST" action="${pageContext.servletContext.contextPath}/${addAction}"  class="form-inline">
						...
						<div class="col-xs-6">
							...
							<select class="selectpicker form-control" name="supplier">
								<c:forEach var="supplier" items="${supplierList }">
									...
									<c:if test="${supplier.id eq product.supplierId}">
